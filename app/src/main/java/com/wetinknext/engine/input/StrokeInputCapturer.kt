...
class StrokeInputCapturer(private val camera: Camera, private val pool: InputBatchPool, private val queue: ArrayBlockingQueue<InputBatch>) {
    ...
    fun onTouchEvent(event: MotionEvent): Boolean = when(event.actionMasked) {
        MotionEvent.ACTION_DOWN, MotionEvent.ACTION_POINTER_DOWN -> { val i=if(event.actionMasked==MotionEvent.ACTION_DOWN) 0 else event.actionIndex; activePointerId=event.getPointerId(i); enqueue(event,InputAction.DOWN,i,false) }
        MotionEvent.ACTION_MOVE -> { val i=event.findPointerIndex(activePointerId); i >= 0 && enqueue(event,InputAction.MOVE,i,true) }
        MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP -> { val i=if(event.actionMasked==MotionEvent.ACTION_UP) 0 else event.actionIndex; if(event.getPointerId(i)!=activePointerId) true else { activePointerId=-1; enqueue(event,InputAction.UP,i,false) } }
        MotionEvent.ACTION_CANCEL -> { activePointerId=-1; enqueueEmpty(InputAction.CANCEL) }
        else -> false
    }
    private fun enqueue(event:MotionEvent, action:InputAction, index:Int, historical:Boolean):Boolean { val batch=pool.acquire()?:return false;batch.begin(action);val t=camera.snapshot();val id=event.getPointerId(index);val tool=tool(event,index);if(historical)for(h in 0 until event.historySize){t.screenToCanvas(event.getHistoricalX(index,h),event.getHistoricalY(index,h),canvasPoint);if(!batch.addSample(canvasPoint[0],canvasPoint[1],pressure(event,tool,index,h),0f,0f,orientation(event,tool,index,h),event.getHistoricalEventTime(h)*1_000_000L,id,tool,true))break};t.screenToCanvas(event.getX(index),event.getY(index),canvasPoint);batch.addSample(canvasPoint[0],canvasPoint[1],pressure(event,tool,index,-1),0f,0f,orientation(event,tool,index,-1),event.eventTime*1_000_000L,id,tool,false);if(!queue.offer(batch)){pool.release(batch);return false};return true }
    private fun enqueueEmpty(action:InputAction):Boolean { val batch=pool.acquire()?:return false;batch.begin(action);if(!queue.offer(batch)){pool.release(batch);return false};return true }
    ...
    private fun pressure(e:MotionEvent,t:PointerTool,i:Int,h:Int):Float=if(t==PointerTool.FINGER||t==PointerTool.UNKNOWN).62f else (if(h>=0)e.getHistoricalPressure(i,h) else e.getPressure(i)).coerceIn(0f,1f)
