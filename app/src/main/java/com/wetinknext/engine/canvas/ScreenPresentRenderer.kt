...
/** Presents a linear RGBA frame target to the default sRGB framebuffer. */
class ScreenPresentRenderer {
    ...
    fun create() {
        ...
        release()
        program = GlProgram(ShaderLib.fullscreenVertex, ShaderLib.linearPresentFragment)
        val currentProgram = checkNotNull(program)
        currentProgram.use()
        textureUniform = GLES30.glGetUniformLocation(currentProgram.id, "uTexture")
        check(textureUniform >= 0) { "Screen present uniform uTexture is missing" }
    ...
    fun render(sourceTextureId: Int, viewportWidth: Int, viewportHeight: Int) {
