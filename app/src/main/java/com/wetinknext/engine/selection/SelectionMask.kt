...
/**
 * Canvas-sized selection mask used by the lasso and the transform preview.
 *
 * The CPU copy (one byte per pixel) owns rasterisation: contour strokes are
 * drawn with [strokePath], closed polygons are scanline-filled, and rectangles
 * / ellipses are filled directly. The GL side is a single R8 texture that
 * shaders sample in canvas UV space (ants overlay, masked transform, cut).
 *
...
class SelectionMask(val width: Int, val height: Int) {
    ...
    fun clear() {
        ...
        textureDirty = true
        isEmpty = true
        selectionBounds[0] = 0; selectionBounds[1] = 0
        selectionBounds[2] = 0; selectionBounds[3] = 0
    }

    // ------------------------------------------------------------ raster ---

    /** Marks a thick capsule stroke between two canvas points. */
    ...
    /** Fills a disk of selection around a canvas point. */
    ...
    /** Fills an axis-aligned rectangle (canvas coords, inclusive). */
    fun fillRect(x0: Int, y0: Int, x1: Int, y1: Int) {
        ...
        for (y in top..bottom) {
            for (x in left..right) mark(x, y)
        }
    }

    /** Fills an ellipse (canvas coords of its bounding box). */
    ...
    /**
     * Even-odd scanline fill of the region enclosed by a drawn contour.
     * Contour runs (from the stroke) toggle membership per row; pixels between
     * pairs of edges are marked.
     */
    fun fillContour() {
        ...
        for (y in 0 until height) {
            ...
            var x = 0
            while (x < width) {
                if (pixels[rowStart + x].toInt() and 0xFF != 0) {
                    edges[edgeCount++] = x
                    while (x < width && pixels[rowStart + x].toInt() and 0xFF != 0) x++
                } else {
                    x++
                }
            }
            var index = 0
            while (index + 1 < edgeCount) {
                for (fillX in edges[index]..edges[index + 1]) mark(fillX, y)
                index += 2
            }
        }
    }
    ...
    private fun include(x: Int, y: Int) {
        if (isEmpty || selectionBounds[2] == 0) {
            selectionBounds[0] = x; selectionBounds[1] = y
            selectionBounds[2] = x + 1; selectionBounds[3] = y + 1
            return
        }
    ...
    /** Deterministic bounds around one contour, or null when empty. */
    fun contourBounds(points: List<FloatArray>): FloatArray? {
        ...
        for (p in points) {
            left = min(left, p[0]); top = min(top, p[1])
            right = max(right, p[0]); bottom = max(bottom, p[1])
        }
    ...
    /** Uploads the CPU copy into the GL texture if anything changed. */
