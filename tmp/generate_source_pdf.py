...
OUTPUT = ROOT / "output" / "pdf" / "wetink-next-source-p0-p31-current.pdf"
...
def footer(canvas, doc):
    ...
    canvas.drawString(16 * mm, 11 * mm, "WetInk Next - source snapshot P0-P31")
...
def main() -> None:
    ...
    doc = SimpleDocTemplate(
        ...
        title="WetInk Next - source snapshot P0-P31",
        author="WetInk Next",
    )
    ...
    story = [
        ...
        Spacer(1, 5 * mm),
        Paragraph("Полный снимок исходного кода: P0-P31", title),
        Spacer(1, 12 * mm),
        Paragraph(
            "Актуализация: P17-P20 - кистевой pipeline и безопасный input; "
            "P21-P31 - жесты, mirror canvas view, FBO и tile-based Undo/Redo; "
            "последний фикс - единый flow для preview и commit RIBBON.",
            body,
        ),
        Spacer(1, 5 * mm),
        Paragraph(f"Включено файлов: {len(files)}. Сгенерировано из рабочей копии проекта.", body),
        PageBreak(),
    ]
    ...
    for number, path in enumerate(files, start=1):
        ...
        numbered = "\n".join(f"{line_no:4d}  {line}" for line_no, line in enumerate(text.splitlines(), 1))
